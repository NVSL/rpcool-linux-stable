...
void vma_to_phys(struct vm_area_struct *vma, unsigned long vaddr)
{
	...
	if (page) {
		...
		printk("[rpcool] vma vaddr: %lu, pfn: %lu, paddr: %pa, long_addr: %lu, "
		       "reverse_pfn: %px\n",
...
char *concat_paths(const char *base, const char *suffix)
{
	...
	char *result = kmalloc(base_len + suffix_len + 2, GFP_KERNEL);
	if (!result)
		return NULL;
	memcpy(result, base, base_len);
	result[base_len] = '/';
	memcpy(result + base_len + 1, suffix, suffix_len);
...
const char *get_path_user(const char __user *path)
{
	...
	result = kmalloc(strlen(fname->name) + 1,
			 GFP_KERNEL); // +1 for null terminator
	if (result) {
...
char *long_to_string(long number, char *buffer, size_t buf_size)
{
	int num_chars = snprintf(buffer, buf_size, "%ld", number);
...
{
	struct filename *fname = getname(base);
    if (IS_ERR(fname)) return NULL;

    char id_str[21]; // Long enough for any 64-bit integer
    if (!long_to_string(connection_id, id_str, sizeof(id_str))) {
        putname(fname);
        return NULL;
    }
    size_t base_len = strlen(fname->name);
    size_t id_len = strlen(id_str);
    size_t suffix_len = strlen(suffix);
    size_t total_length = base_len + 1 + id_len + 1 + suffix_len + 1; // base + '/' + id + '/' + suffix + '\0'
    char *result = kmalloc(total_length, GFP_KERNEL);
    if (!result) {
        putname(fname);
        return NULL;
    }

    char *ptr = memcpy(result, fname->name, base_len) + base_len;
    *ptr++ = '/';
    ptr = memcpy(ptr, id_str, id_len) + id_len;
    *ptr++ = '/';
    memcpy(ptr, suffix, suffix_len + 1); // Include '\0' in the copy

    putname(fname); // Clean up
...
const char *concat_dev_connection_paths_user(const char *dev_prefix,
					     const char __user *base,
					     long connection_id,
					     const char *suffix)
{
	printk("[rpcool] concat_dev_connection_paths_user called\n");
	const char *tmp =
		concat_connection_paths_user(base, connection_id, suffix);
