...
int reset_seal_nonce_at_index(struct SealStore* seal_store, int index) {
    ...
    pos = ENTRIES_START_POS + index * sizeof(struct SealEntry) + sizeof(entry.addr) + sizeof(entry.len);
    result = kernel_write(seal_store->f_metadata, &entry.nonce, sizeof(entry.nonce), &pos); // also stores the new nonce
    if (result < 0) {
        pr_err("[rpcool] Error writing entry to queue: %zd\n", result);
        return result;
    }
